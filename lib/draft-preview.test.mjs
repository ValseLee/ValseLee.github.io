...
test("draft slugs exclude malformed percent encoding without rejecting reserved characters", async (t) => {
  const { listDrafts, loadDraft } = await subjectPromise;
  const root = fixtureRoot(t);
  const source = `---\ntitle: Encoded Draft\ndate: 2026-07-22\ncategory: engineering\ntags: []\nlinks: []\ndescription: Encoded filename\n---\n\nDraft body\n`;
  for (const slug of ["draft?sentinel", "literal%3Fslug", "literal%value"]) {
    fs.writeFileSync(path.join(root, ".article-drafts", `${slug}.mdx`), source);
  }

  assert.deepEqual(
    listDrafts({ root, nodeEnv: "development" }).map(({ slug }) => slug),
    ["draft?sentinel", "literal%3Fslug"],
  );
  assert.equal(loadDraft("articles", "draft?sentinel", { root, nodeEnv: "development" }).status, "ok");
  assert.equal(loadDraft("articles", "literal%3Fslug", { root, nodeEnv: "development" }).status, "ok");
  assert.deepEqual(loadDraft("articles", "literal%value", { root, nodeEnv: "development" }), { status: "missing" });
});
